import { tokenizePattern } from "./pattern-highlighter.js";
export function tokenizeCss(lines, options = {}) {
    const tokens = tokenizePattern(lines, "css", options);
    applyCssColorSwatches(lines, tokens);
    return tokens;
}
export const cssHighlighter = {
    id: "css",
    name: "CSS",
    extensions: ["css"],
...
function applyCssColorSwatches(lines, tokens) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (const match of line.matchAll(colorRe)) {
            ...
            const text = match[0];
            if (text.startsWith("#") && ![4, 5, 7, 9].includes(text.length))
                continue;
