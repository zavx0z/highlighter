...
describe("tokenizeTypeScript", () => {
  ...
  test("highlights object keys and typed parameter names", () => {
    ...
    const tokens = tokenizeTypeScript(lines, {
      resolveForeground(scopes) {
        return scopes.includes("storage.type") ? "#cf8e6d" : undefined
      },
    })
  ...
  test("is theme-neutral until a foreground resolver is provided", () => {
    const line = "const value: number = 1"
    const tokens = tokenizeTypeScript([line])

    expect(tokens[0]?.every((token) => token.fg === undefined)).toBe(true)
  })
})
