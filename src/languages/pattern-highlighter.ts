...
export function tokenizeSourcePattern(
  ...
  if (isJsonPath(opts.path)) return tokenizeJsonPattern(lines, opts)
  if (isSqlitePath(opts.path)) return tokenizeSqlitePattern(lines, opts)
  if (isMarkdownPath(opts.path)) return tokenizeMarkdownPattern(lines, opts)
  return tokenizeTypeScriptPattern(lines, opts)
}

export function tokenizePatternRangeTokens(
  source: string,
  base: number,
  language: PatternLanguageId,
  options: TokenizeOptions = {},
...
export function tokenizePatternRanges(
  source: string,
  base: number,
  language: PatternLanguageId,
  push: (s: number, e: number, c: string, bg?: string, fg?: string) => void,
  options: TokenizeOptions = {},
): void {
...
function flattenPatternTokens(
  stream: PatternTokenStream,
  start: number,
  inheritedTypes: readonly string[] | undefined,
  out: RangeToken[],
  ...
  if (Array.isArray(stream)) {
    let cursor = start
    for (const item of stream) cursor = flattenPatternTokens(item, cursor, inheritedTypes, out, resolveForeground)
    return cursor
  }
...
function colorForTypes(types: readonly string[], resolveForeground: ResolveForeground | undefined): string | undefined {
  if (resolveForeground === undefined) return undefined
...
function applySemanticIdentifierOverlays(
  source: string,
  base: number,
  tokens: RangeToken[],
  resolveForeground: ResolveForeground | undefined,
...
function applyObjectKeyOverlays(
  source: string,
  base: number,
  tokens: RangeToken[],
  resolveForeground: ResolveForeground | undefined,
...
function applyParameterOverlays(
  source: string,
  base: number,
  tokens: RangeToken[],
  resolveForeground: ResolveForeground | undefined,
...
function applyMemberAccessOverlays(
  source: string,
  base: number,
  tokens: RangeToken[],
  resolveForeground: ResolveForeground | undefined,
...
function pushMemberPropertyToken(
  base: number,
  tokens: RangeToken[],
  accessorStart: number,
...
function applyTemplateLiteralOverlays(
  ...
  for (const template of templates) pushTemplateLiteralTokens(source, base, tokens, template, contentLanguage, options)
}
...
function pushTemplateExpressionTokens(
  source: string,
  base: number,
  tokens: RangeToken[],
  start: number,
...
function applyDefaultIdentifierOverlays(
  source: string,
  base: number,
  tokens: RangeToken[],
  resolveForeground: ResolveForeground | undefined,
...
function tokenizeMarkdownFenceContent(
  lines: readonly string[],
  info: string,
  options: TokenizeOptions,
): EditorTokens {
  ...
  if (language === "xml") return tokenizeXmlPattern(lines, options)
  if (language === "json") return tokenizeJsonPattern(lines, options)
  if (language === "sql") return tokenizeSqlitePattern(lines, options)
  return lines.map(() => [])
}
